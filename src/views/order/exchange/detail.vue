    ...
    <el-card shadow="never">
      ...
      <el-table ref="productTable" border class="standard-margin" :data="productList">
        ...
        <el-table-column label="商品名称" align="center">
          <template slot-scope="scope">
            <span class="font-small">{{ scope.row.productName }}</span><br> <span class="font-small">品牌：{{ scope.row.productBrand }}</span>
          </template>
        </el-table-column>
        <el-table-column label="价格/货号" width="180" align="center">
          <template slot-scope="scope">
            <span class="font-small">价格：￥{{ scope.row.productRealPrice }}</span><br> <span class="font-small">货号：NO.{{ scope.row.productId }}</span>
          </template>
        </el-table-column>
      ...
      <div style="float:right;margin-top:15px;margin-bottom:15px">
        <span class="font-title-medium">合计：</span> <span class="font-title-medium color-danger">￥{{ totalAmount }}</span>
      </div>
    </el-card>
      ...
      <div class="form-container-border">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" style="height:52px" :span="18">
            ￥
            <el-input v-model="updateStatusParam.returnAmount" size="small" :disabled="orderReturnApply.status!==0" style="width:200px;margin-left: 10px" />
          </el-col>
        </el-row>
...
<script>
import { getApplyDetail, updateApplyStatus } from '@/api/order/exchange'
